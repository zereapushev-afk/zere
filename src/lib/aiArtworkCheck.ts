...
export const AI_BLOCK_THRESHOLD = 80;
...
export async function checkArtwork(file: File): Promise<AiCheckResult> {
  ...
  const { data, error } = await supabase.functions.invoke('ai', {
    body: {
      ...
      system: `Ты строгий модератор, который ищет ИИ-генерацию. Ложный пропуск опаснее ложной блокировки: сомнительные изображения должны получать не меньше 80. Сначала оцени общую вероятность aiLikelihood 0..100.
...
"типичный стоковый мультяшный персонаж". Особенно внимательно проверяй мультяшных животных, лягушек и персонажей с гладкими градиентами, пластиковым объёмом и шаблонным фоном. Не считай симметрию и аккуратность доказательством ручной работы.
...
"styleSignals":["точные строки из перечня"],"reasons":["признаки на русском"]}.`,
  ...
  return {
    score,
    blocked: score >= AI_BLOCK_THRESHOLD,
